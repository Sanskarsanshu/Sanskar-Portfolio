...
export function CalendlyCarousel({
  ...
  // Handle empty items case gracefully
  ...
  }, [page, pauseOnHover, isHovered, autoPlayInterval, total]);
        ...
        {VISIBLE_OFFSETS.map((offset) => {
          ...
          return (
            <motion.div
              ...
              className={cn(
                "rounded-[28px] sm:rounded-[32px] bg-black/20 border border-ice-600/30 backdrop-blur-xl text-ice-200 shadow-[0_10px_30px_rgba(0,0,0,0.4),0_4px_12px_rgba(0,0,0,0.6)] overflow-visible",
            ...
            >
              {offset === -1 && (
                <div
                  aria-hidden="true"
                  className="absolute top-0 bottom-0 flex items-center text-ice-900/60 pointer-events-none z-[100]"
                  style={{
                    width: 22,
                    height: 42,
                    margin: "auto 0",
              ...
              >
                ...
                >
                  <div className="size-full rounded-[20px] sm:rounded-[24px] overflow-hidden bg-black/40 border border-white/5 relative">
                    <Image
                ...
                >
                  ...
                  >
                    <div className="flex-1 min-w-0 flex flex-col items-center md:items-start text-center md:text-left justify-between py-1 gap-2 sm:gap-3">
                      <h3
                        title={item.stat}
                        className="text-lg sm:text-2xl md:text-3xl lg:text-4xl font-bold tracking-tight text-white leading-tight w-full"
                      >
                        {item.stat}
                      </h3>
                      ...
                      <div className="relative w-full min-w-0 my-auto py-1">
                        <span
                          aria-hidden="true"
                          className="font-serif text-2xl sm:text-3xl lg:text-4xl text-ice-500/40 absolute right-full top-0 pr-1 select-none pointer-events-none hidden md:inline"
                        >
                          "
                        </span>

                        <p className="text-xs sm:text-base md:text-lg text-ice-300 leading-snug font-mono">
                          "{item.quote}"
                        </p>
                      </div>

                      <div className="flex min-w-0 w-full max-w-full overflow-hidden items-center md:items-start justify-center md:justify-start">
                        <div className="flex flex-col items-center md:items-start min-w-0 max-w-full gap-2">
                          <span className="w-fit inline-flex items-center justify-center rounded-[4px] font-medium py-1 px-2.5 text-[11px] sm:text-xs bg-ice-600/30 text-ice-200 border border-ice-600/20 shrink-0 select-none">
                            <span className="whitespace-nowrap font-semibold uppercase tracking-wider font-mono">
                              {item.author}
                            </span>
                          </span>

                          <span className="w-fit inline-flex items-center justify-center rounded-[4px] font-medium py-1 px-2.5 text-[10px] sm:text-xs bg-ice-600/10 border border-ice-600/10 text-ice-400 max-w-full select-none">
                            <span
                              title={item.role}
                              className="truncate uppercase tracking-widest font-mono"
                            >
                              {item.role}
                            </span>
                          </span>
                    ...
                    <div className="relative shrink-0 overflow-hidden rounded-[18px] sm:rounded-[22px] bg-black/20 border border-white/5 w-full md:w-[clamp(180px,44%,330px)] flex-1 md:flex-initial md:h-full max-h-[220px] md:max-h-none">
                      <Image
                        alt={item.alt || item.author}
        ...
        {items.map((item, idx) => {
          ...
          return (
            <button
              ...
              className={cn(
                "h-[8px] rounded-[3px] overflow-hidden border border-white/10 p-0 cursor-pointer transition-[width] duration-300 ease-out outline-none focus-visible:ring-2 focus-visible:ring-ice-500",
                isSelected
                  ? "w-[80px] bg-white/20"
                  : "w-[8px] bg-white/5 hover:bg-white/10"
              )}
            >
              {isSelected && (
                <div
                  className="h-full rounded-[3px] bg-ice-300"
                  style={{
